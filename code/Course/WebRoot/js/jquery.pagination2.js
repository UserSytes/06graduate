	...
	this.each(function(){
		...
		function render(){
			...
			if(currPage>1){
				imgs[0]="/first.gif"
				imgs[1]="/prev.gif";
				jQuery("#first",panel).click(function(){
					optPage(1);
				});
			...
			if(currPage < pages){
				imgs[2]="/next.gif";
				imgs[3]="/last.gif";
				jQuery("#next",panel).bind("click",function(){
					optPage(currPage+1);
				});
				jQuery("#last",panel).bind("click",function(){
					optPage(pages);
				});
				css[1]="disabledTrue";
			}
			jQuery("input.go").click(function(){
				try{
					var turn=parseInt(jQuery("#turn").val());
			...
			jQuery.each(imgs,function(i,n){
				var obj=jQuery(ids[i]).attr("src",imgPath+n).addClass(css[Math.floor(i/2)]);
			});
		}
		render();
	});
